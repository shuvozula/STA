...
import random
import math
from createTaskScheduleIDASTAR import *

def printit(msg, showInput):
    if showInput:
        print msg

def createData(numRobots, maxBidsPerTask, coalitionSize, bidLimit, taskSize, maxBidValue, fixedCoalitionSize, showInput):
        ...
        while (doublePositive):
            ...
            while (taskCountExceeded):
                ...
                if ((arrBidsPerTaskCount[arrTasks.index(task)]+1) <= maxBidsPerTask):
                    ...
                    taskCountExceeded = True
                    printit("task count exceeded: " + str(arrBidsPerTaskCount), showInput)
    
            
            if (fixedCoalitionSize):
                randSize = coalitionSize[1] # use the high range
            else:
...
#            print "**** ||" + str(line) + "||"
            ...
            if (dblPos):
                printit(str(line) + " ---> bid is being repeated, skipping....", showInput)
            else:
                ALLBIDS.append(line)
                line += str(random.randint(1,maxBidValue))
                doublePositive = False
                printit(line, showInput)
...
#    print "#_#_#_#_#"
    printit("- - - - - - - - - - - - - - - - -", showInput)
    printit("Count of the bids per task: " + str(arrBidsPerTaskCount), showInput)
    printit("----------------------------------------------------", showInput)
    
    printit(dataCollection, showInput)
